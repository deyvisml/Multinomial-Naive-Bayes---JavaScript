...
class MultinomialNB {
  ...
  predict(X) {
    ...
    for (const x of X) {
      ...
      for (const class_name of this.class_names) {
        //console.log("log_prior", this.log_prior[class_name]);
      ...
      // modifying: reduce true negative rate
      let threshold = 1.025; // (umbral) 1 is default, but a good value is around 1.03
      pred = "0";
